...
module Swicky
  ...
  class Notebook
    ...
    def data
      @data ||= Query.new(N::URI).select(:s, :p, :o).distinct.where(:s, :p, :o, url).execute
    end
    ...
    def delete
      FederationManager.delete(nil, nil, nil, url)
      FederationManager.delete(user_url, N::TALIA.hasSwickyNotebook, url)
      FederationManager.delete(url, N::RDF.type, N::TALIA.SwickyNotebook)
    end
    
    def load(xml_file)
      @data = nil
      begin
        ConnectionPool.write_adapter.load(xml_file, 'rdfxml', url)
      rescue Exception => e
        puts "\tProblem loading #{xml_file.to_s}: (#{e.message}) File not loaded!"
        puts e.backtrace
      end
      FederationManager.add(user_url, N::TALIA.hasSwickyNotebook, url)
      FederationManager.add(url, N::RDF.type, N::TALIA.SwickyNotebook)
    end
    ...
    def exist?
      Query.new(N::URI).select(:user).where(:user, N::TALIA.hasSwickyNotebook, url).execute.size > 0
    end
    
    class << self
      def find_all(user_name = nil)
        nb_query = Query.new(N::URI).select(:notebook).distinct
      ...
      def coordinates_for(url)
        ...
        frag_qry = Query.new(N::URI).select(:coordinates).distinct
      ...
      def select_annotations(*note_matching)
        ...
        note_triples_qry = Query.new(N::URI).select(:note, :predicate, :object).distinct
        ...
        statement_triples_qry = Query.new(N::URI).select(:statement, :predicate, :object).distinct
        ...
        result_triples.each do |trip|
          additional_triples += Query.new(N::URI).select(:predicate, :object).where(trip[1].to_uri, :predicate, :object).execute.collect { |result| [trip[1].to_uri] + result }
          if(trip.last.respond_to?(:uri))
            additional_triples += Query.new(N::URI).select(:predicate, :object).where(trip.last, :predicate, :object).execute.collect { |result| [trip.last] + result }
          end
        end
